...
public:
	...
	// event
	/*
		Function: on_ChatCommand
			Called when a chat message begins with "/"

		Arguments:
			ClientID - The player that sent the message.
			pCommand - The command of the player. The first word before a whitespace
			pParams - Everything after the first whitespace ^

        Returns:
            false if the message will be sent to chat
            true if the message will NOT be sent to the chat (on successfull command handling)
	*/
    virtual bool OnChatCommand(int ClientID, const char* pCommand,const char* pParams);
	/*
