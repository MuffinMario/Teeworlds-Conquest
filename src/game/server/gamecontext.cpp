...
void CGameContext::SendChat(int ChatterClientID, int Mode, int To, const char *pText)
{
	...
	else if(Mode == CHAT_COMMAND)
    {
        Msg.m_Mode = CHAT_ALL; //heh
		...
		if(MsgID == NETMSGTYPE_CL_SAY)
		{
			...
			if(*p == '/')
            {
                ...
                for(int i = 0;i <= 127;i++)
                {
                    ...
                    {
                        /* Command is split on space */
                        if(p[i] == ' ')
                        {
                            CommandSize = p + i - pCommand ;
                            pParams = p + i + 1;
                            break;
                        }
                    }
                }
                const char aCommandStr[129] = {0};
                if(CommandSize != -1)
                {
                    strncpy((char*)aCommandStr,pCommand,CommandSize);
                    pCommand = aCommandStr;
                }
                if(pParams == nullptr)
                    pParams = "";

                /* Don't send chat message if chat command is successfull */
                if(m_pController->OnChatCommand(ClientID,pCommand,pParams))
                    return;
            }
