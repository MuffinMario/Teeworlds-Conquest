...
CProjectileGCQ::CProjectileGCQ(CGameWorld *pGameWorld, int Type, int Owner, vec2 Pos, vec2 Dir, int Span,
...
void CProjectileGCQ::Reset()
{
	GameServer()->m_World.DestroyEntity(this);
}

vec2 CProjectileGCQ::GetPos(float Time)
{
	float Curvature = 0;
	float Speed = 0;
...
void CProjectileGCQ::Tick()
{
	...
	if(TargetChr || Collide || m_LifeSpan < 0 || GameLayerClipped(CurPos))
	{
		...
		if(m_Explosive)
        {
            GameServer()->CreateDEATHExplosion(CurPos, m_Owner, m_Weapon, m_Damage);
        }
...
void CProjectileGCQ::TickPaused()
{
	++m_StartTick;
}

void CProjectileGCQ::FillInfo(CNetObj_Projectile *pProj)
{
...
void CProjectileGCQ::Snap(int SnappingClient)
{
	float Ct = (Server()->Tick()-m_StartTick)/(float)Server()->TickSpeed();
